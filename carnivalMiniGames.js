...
function soundManager(sA) {
  ...
  if (sA === 'mHub') {
    ...
    if(scene === 1 && wplSound.isPlaying() && roundEnded === false) {
      wplSound.stop();
    }
  ...
  if (sA === 'grLv' ){
    ...
    if(!mlSound.isPlaying() && !wplSound.isPlaying() && !grlSound.isPlaying()) {
      grlSound.play();
    }
